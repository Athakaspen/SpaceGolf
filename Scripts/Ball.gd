...
func init(id:int, spawn_point:Vector2, grav_bit:int, mode:String):
	name = str(id)
...
func _process(_delta):
	...
	if is_network_master():
		trajline.visible = isGrounded and isMyTurn
...
func _physics_process(_delta):
	if is_network_master():
		if (Input.is_action_just_pressed("fire") and isGrounded and isMyTurn):
